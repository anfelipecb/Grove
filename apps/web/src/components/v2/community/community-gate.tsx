import { COMMUNITY_UNLOCK_GLOBAL_LEVEL, getGlobalLevel, hasCommunityAccess } from "@grove/core";
...
export function CommunityGate({ totalXp }: Props) {
  ...
  const xpToUnlock = Math.max(0, 250 - totalXp);
  const progress =
    globalLevel >= COMMUNITY_UNLOCK_GLOBAL_LEVEL
      ? 100
      : Math.min(100, Math.round((totalXp / 250) * 100));
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col items-center px-6 py-16 text-center">
      ...
      <h1 className="text-xl font-semibold text-foreground">Community unlocks at Sprout</h1>
