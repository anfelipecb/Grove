...
export function LandingExperience({ showDemoLinks = false }: { showDemoLinks?: boolean }) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-ink">
      <div
        className="pointer-events-none absolute -left-32 top-20 h-96 w-96 rounded-full bg-moss/25 blur-[100px] motion-safe:animate-landing-float"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute -right-24 top-40 h-[28rem] w-[28rem] rounded-full bg-fern/80 blur-[90px] motion-safe:animate-landing-float-slow"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute bottom-0 left-1/3 h-72 w-72 rounded-full bg-marigold/20 blur-[80px] motion-safe:animate-landing-drift"
        aria-hidden="true"
      />
      <div
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col gap-16 px-4 py-12 sm:px-6 lg:gap-20 lg:px-8 lg:py-16">
        <header className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/50 bg-white/35 shadow-glass backdrop-blur-md">
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:justify-end">
            <Link
              href="/sign-in"
              className="rounded-xl border border-white/40 bg-white/30 px-4 py-2.5 text-sm font-semibold text-bark shadow-glass backdrop-blur-md transition hover:bg-white/50 hover:shadow-lg"
            >
              Sign in
            </Link>
            <Link
              href="/sign-up"
              className="inline-flex items-center gap-2 rounded-xl border border-moss/30 bg-moss/90 px-4 py-2.5 text-sm font-semibold text-white shadow-glass backdrop-blur-sm transition hover:bg-bark"
            >
              Get started
        ...
        <section className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border border-white/45 bg-white/25 px-3 py-1 text-xs font-medium text-bark shadow-sm backdrop-blur-md">
              <Network className="h-3.5 w-3.5 text-moss" aria-hidden="true" />
              Self-improvement, coordinated—not automated away
            </p>
            <h1 className="mt-6 text-4xl font-semibold leading-[1.1] tracking-tight text-bark sm:text-5xl lg:text-[3.25rem]">
              ADHD-aware accountability that keeps you connected—not alone.
            </h1>
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-stone-700/95">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link
                href="/sign-in"
                className="inline-flex items-center gap-2 rounded-xl border border-white/50 bg-white/35 px-5 py-3.5 text-sm font-semibold text-bark shadow-glass backdrop-blur-md transition hover:bg-white/55"
              >
          ...
          <div className="rounded-2xl border border-white/50 bg-white/35 p-6 shadow-glass backdrop-blur-xl transition hover:bg-white/45 hover:shadow-xl sm:p-8">
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold transition ${
                      isOn
                        ? "border-moss/50 bg-moss/15 text-bark shadow-sm"
                        : "border-white/40 bg-white/25 text-stone-700 hover:bg-white/40"
                    }`}
                  >
            ...
            <div
              key={panel.id}
              className="mt-6 rounded-xl border border-white/35 bg-white/40 p-5 shadow-inner backdrop-blur-sm motion-safe:animate-fadeIn"
            >
              <h2 className="text-lg font-semibold text-bark">{panel.headline}</h2>
              <p className="mt-3 text-sm leading-relaxed text-stone-700">{panel.body}</p>
            </div>
        ...
        <section className="grid gap-5 md:grid-cols-3">
          {[
            ...
            return (
              <article
                key={card.title}
                className={`group relative overflow-hidden rounded-2xl border border-white/45 bg-gradient-to-br ${card.cardClass} p-6 shadow-glass backdrop-blur-md transition duration-300 hover:-translate-y-1 hover:border-white/60 hover:shadow-xl`}
              >
                <div className="relative">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/50 bg-white/50 text-bark shadow-sm backdrop-blur-sm transition group-hover:scale-105">
                    <Icon className="h-5 w-5" aria-hidden="true" />
                  </div>
                  <h2 className="mt-4 text-lg font-semibold text-bark">{card.title}</h2>
                  <p className="mt-2 text-sm leading-relaxed text-stone-700">{card.text}</p>
                </div>
              </article>
            );
          })}
        ...
        <footer className="border-t border-white/30 pt-10 text-center text-xs text-stone-500">
