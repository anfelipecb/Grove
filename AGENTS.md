...
- **Never `git stash drop` on the main checkout.** Uncommitted agent work on `master` is often untracked and is not recoverable after `drop`. To pull `master`, commit or branch first, or use `git stash pop` only when you intend to restore. Prefer worktrees so the main checkout stays clean.
